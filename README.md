...
- PDF-таблицы с распознаваемыми границами извлекаются по координатам ячеек;
  таблицы DOCX сохраняются на своих местах. Сканы требуют внешнего OCR.
  Для старых ревью с плоским текстом нужно заново загрузить оригинал: сохранённые
  снимки не переписываются, чтобы не сместить цитаты и историю правок;
- независимые фильтры по серьёзности и статусу замечаний, поиск и счётчик результатов;
- предзаполненный демо-вход и выбор одного из трёх готовых PDF: `TZ_01`, `TZ_02`,
  `TZ_03`; выбранный оригинал сохраняется вместе с ревью;
- единый WYSIWYG-редактор текста и ячеек таблиц с переходом черновика к итогу без
  запуска повторного ревью;
- табличное представление документа в браузерной печати/PDF;
- круговая диаграмма повторяющихся подтверждённых ошибок с долями и количеством;
- семантически некорректный ответ критика/судьи запускает резервную модель;
  если обе модели не справились, отчёт остаётся явно неполным;
...
```mermaid
flowchart LR
    DOC["ТЗ<br/>весь текст целиком"]

    DOC --> A1["Аналитик<br/>постановка · термины · объёмы"]
    DOC --> A2["Data Engineer<br/>поля · join · загрузка"]
    DOC --> A3["Архитектор<br/>противоречия · NFR"]
    DOC --> A4["QA<br/>приёмка · граничные случаи"]

    A1 --> CRITIC["Критик<br/>отклоняет недоказанное<br/>и домысленное"]
    A2 --> CRITIC
    A3 --> CRITIC
    A4 --> CRITIC

    CRITIC --> JUDGE["Судья<br/>склеивает дубли"]
```
