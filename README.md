...
Каждый узел схемы — LLM-агент Cloud.ru Foundation Models со своим системным
...
| Критик | `CloudRuEvidenceCritic` | `CRITIC_SYSTEM_PROMPT` | Гасит ложные срабатывания: цитата обязана подтверждать проблему |
...
2. перед вызовом критика замечания без дословной цитаты в документе отбрасываются
   без обращения к модели — это дешёвая защита от галлюцинаций, а не отдельный
   агент;
3. LLM-критик перепроверяет оставшееся, отклоняет домысленное и снижает
   уверенность спорных замечаний;
4. LLM-судья сводит смысловые дубли разных ролей и переоценивает severity;
5. статус документа считается по максимальной тяжести оставшихся замечаний.
