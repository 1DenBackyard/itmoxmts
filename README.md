...
Каждый узел схемы реализован в коде. Ролевые агенты работают параллельно, затем их
замечания проходят два контура контроля.
...
| Аналитик | `AnalystReviewer`, `CloudRuLLMReviewer` | `ROLE_BRIEFS["Аналитик"]` | Постановка, термины, заглушки шаблона, объёмы, регламент |
| Data Engineer | `DataEngineerReviewer`, `CloudRuLLMReviewer` | `ROLE_BRIEFS["Data Engineer"]` | Контракт полей, join, агрегация, стратегия загрузки, идемпотентность |
| Архитектор | `ArchitectReviewer`, `CloudRuLLMReviewer` | `ROLE_BRIEFS["Архитектор"]` | Противоречия разделов, недостижимые ветки, зависимости, NFR |
| QA | `QAReviewer`, `CloudRuLLMReviewer` | `ROLE_BRIEFS["QA"]` | Тестируемость, граничные и негативные сценарии, детерминированность |
| Критик | `EvidenceCritic`, `CloudRuEvidenceCritic` | `CRITIC_SYSTEM_PROMPT` | Гасит ложные срабатывания: цитата обязана быть в документе дословно |
| Судья | `IssueJudge`, `CloudRuIssueJudge` | `JUDGE_SYSTEM_PROMPT` | Сводит дубли разных агентов, выставляет итоговую тяжесть и порядок |

Конвейер одного ревью:

1. ролевые агенты (детерминированные + LLM) параллельно возвращают кандидатов;
2. детерминированный критик отбрасывает замечания без дословной цитаты и с
   `confidence < 0.55`;
3. LLM-критик перепроверяет оставшееся и снижает уверенность спорных замечаний;
4. детерминированный судья дедуплицирует по категории и заголовку;
5. LLM-судья объединяет смысловые дубли разных ролей и переоценивает severity;
6. статус документа считается по максимальной тяжести оставшихся замечаний.

Каждый LLM-шаг необязателен: при недоступности модели агент добавляет запись в
`warnings`, а ревью продолжается на детерминированном контуре. Промпты собраны в
[prompts.py](src/specguard/review/prompts.py): общие правила доказательности,
шкалы `severity` и `confidence` и защита от инструкций внутри документа общие для
всех ролевых агентов, различаются только зоны ответственности.
...
│   └── review/
│       ├── agents.py              # Детерминированные ролевые проверки
