...
if not PROTO.is_dir():
    raise SystemExit(
        "Архивный генератор: требуется удалённый из исходной ветки prototype/src. "
        "Для запуска HTML используйте готовый web/data/dataset.json."
    )
...
def enrich_flow(text: str) -> str:
    ...
    text = re.sub(
        r"Таблица: TABLE_GEO_RAW_3G\s*Поле Тип данных Описание"
        r"[\s\S]*?(?=Таблица: TABLE_GEO_RAW_4G)",
    ...
    text = re.sub(
        r"Таблица: TABLE_GEO_RAW_4G\s*Поле Тип данных Описание"
        r"[\s\S]*?(?=Дополнительная информация|FAQ|$)",
...
def blank_template(doc_type: str) -> str:
    ...
    parts = []
    for b in BLOCK_DEFS:
        if doc_type in b.required_for or b.id in core:
